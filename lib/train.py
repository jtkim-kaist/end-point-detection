...
def main(argv=None):
    ...
    with tf.device(config.device):
        with tf.variable_scope("model", reuse=None):
            m_train = trnmodel.Model(is_training=True, global_step=global_step,keep_prob=0.5)
        with tf.variable_scope("model", reuse=True):
            m_valid = trnmodel.Model(is_training=False, global_step=global_step,keep_prob=1.0)
    ...
    list_var_teacher = tf.get_collection(key=tf.GraphKeys.GLOBAL_VARIABLES, scope="model/CNN")
